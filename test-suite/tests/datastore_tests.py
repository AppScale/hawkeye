...
class SimpleKindAwareQueryTest(HawkeyeTestCase):
  def runTest(self):
    ...
    for entry in project_list:
      status, headers, payload = self.http_get('/datastore/project?id={0}'.
    ...
    for entry in module_list:
      status, headers, payload = self.http_get('/datastore/module?id={0}'.
...
class QueryByKeyNameTest(HawkeyeTestCase):
  def runTest(self):
    ...
    status, headers, payload = self.http_get('/datastore/entity_names?' \
                                             'project_name={0}'.
...
class LimitedResultQueryTest(HawkeyeTestCase):
  def runTest(self):
    ...
    entity_list = self.assert_and_get_list('/datastore/project_ratings?'
                                           'rating=6&comparator=ge&limit=2&desc=true')
    self.assertEquals(len(entity_list), 2)
    last_rating = 100
    for entity in entity_list:
      self.assertTrue(entity['rating'], last_rating)
...
class SimpleTransactionTest(HawkeyeTestCase):
  def runTest(self):
    key = str(uuid.uuid1())
    status, headers, payload = self.http_get('/datastore/transactions?' \
